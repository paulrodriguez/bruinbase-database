...
typedef struct {
  ...
  int     eid;  //the first entry in the node is eid=1
} IndexCursor;
...
class BTreeIndex {
 public:
  ...
  /**
	this locates the leaf where we are supposed to insert a new node node if the height of tree is greater than or equal to 2
	@param int key[IN] the key to insert to the leaf
	@param const RecordId& rid[IN] the rid to insert to the leaf
	@param PaheId currnetPid[IN] the page id of the node we need to search to find the leaf and insert the key
	@param int level [IN] the current level of the tree we are in. the root is level 1 and the leafs are at level=treeHeight
	@param PageId& siblingPid[IN] the pid of a sibling pid if a split occured at the leaf
	@param int& sib_mid_key[IN] the sibling key returned when we split a leaf node, or the middle key returned when splitting a nonleaf node
	@return 0 if no error. error code
  
  **/
  RC locateAndInsertToLeaf(int key,  const RecordId& rid, PageId currentPid, int level, PageId& siblingPid, int& sib_mid_key);
  RC readdata();
  RC writedata();
 private:
