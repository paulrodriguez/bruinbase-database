...
class BTLeafNode {
  public:
	/*each PageFile is 1024 bytes.
	  4 bytes are used at the beginning to store the numebr of records in the file
	  the leaf stores a RecordId, which is 8 bytes since it's two integes, and a key which is 4 bytes
	  so each record is 12 bytes long. each node is stored in a page
	  4 bytes are used at the end to store the pageid
	  to get the pageid will first have to get number of records
	*/

	static const int MAX_LEAF = 84;
	static const int LEAF_ENTRY_SIZE = 12;
	/**
...
class BTNonLeafNode {
  public:
  //first four bytes contain the number of nodes, the last 
  static const int MAX_NON_LEAF = 100;
  static const int PAIR_SIZE = 8;
  /**
  * constructor that initializes the node
  **/
  BTNonLeafNode();
  
  RC startPid(PageId pid);
   /**
	...
	**/
	RC locateKey(int searchKey, int& eid);
